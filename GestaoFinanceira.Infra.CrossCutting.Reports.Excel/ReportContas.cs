...
namespace GestaoFinanceira.Infra.CrossCutting.Reports.Excel
{
    public class ReportContas
    {
        public static byte[] Get(List<Conta> contas) {
            ...
            using(var excel = new ExcelPackage())
            {
                ...
                foreach (var item in contas)
                {
                    ...
                    if (linha % 2 == 0)
                    {
                        var conteudo = sheet.Cells[$"A{linha}:D{linha}"];
                ...
                var tabela = sheet.Cells[$"A3:D{linha - 1}"];
